...
ui <- fluidPage(
  headerPanel('Conditional probability comparison'),
  sidebarPanel('Input parameters for events of interest. ',
    ...
    numericInput(inputId="event.b", label='Occurrences of event B', 500,
                 min = 0),
    numericInput(inputId="population", label='Population size', 1000,
                 min = 0),
    h4("Comparison event (red line): probability(died in USA in 2010|85 or older in USA in 2010)"),
    h6("Number of deaths in USA in 2010 (event A): 2,550,000"),
    h6("Population over 85 in USA in 2010 (event B): 5,176,000"),
    h6("Total US population in 2010 (population size): 310,000,000"),
...
server <- function(input, output) {
  ...
  output$plot1 <- renderPlot({
    plot(pmin(P.b.given.a*((input$event.a/input$population)/(input$event.b/input$population)),ones),type="l",main="Probability of A given B", xlab="Assumed P(B|A)", ylab="P(A|B)",xaxt="n", ylim=c(0,1))
 axis(1,at=1:101,labels=P.b.given.a)
    lines(baseline,col=2)
     })
